...
class GaussianMLE(MaximumLikelihoodEstimatorBase):
    ...
    def mle_estimate(self, coeffs: tuple):
        ...
        return - (-(n/2)* np.log(2*np.pi) - (n/2)*np.log(sigma**2) - (1/(2*sigma**2))*sum((self.y_data - mu)**2))
    ...
    def mle_regression(self, coeffs: tuple):
        ...
        # log-likelihood function of Poisson distribution, with parameter substituted for inverse link transformation.
        return - (-(n/2)* np.log(2*np.pi) - (n/2)*np.log(sigma**2) - (1/(2*sigma**2))*sum([(self.y_data[i] - (alpha + self.x_data[i]*beta))**2 for i in range(n)]))
...
def poisson_fit():
    mle = PoissonlMLE(x_data=[70, 80, 90, 50, 60, 40, 50, 60, 80, 90, 10, 20, 30, 10],
...
def gaussian_fit():
    mle = GaussianMLE(x_data=[70, 80, 90, 70, 75, 80, 10],
                      y_data=[7, 8, 9, 7, 7.5, 8, 1])
    mle.fit_estimate()
    print(mle.estimate_model)
